...
Macha's small HTTP server does not currently provide a web application or a CORS layer. The easiest web deployment is same-origin proxying:
...
The current Macha server has no playback, stream, remux or transcode endpoint. `media_ids` identify associated media but are not themselves browser-playable URLs.

The client therefore defines only this boundary:

```ts
interface PlaybackResolver {
  readonly available: boolean;
  resolve(media, capabilities): Promise<PlaybackSource>;
}
```

A later C++ server implementation can satisfy it with whatever final HTTP contract is chosen. No current endpoint is assumed.
