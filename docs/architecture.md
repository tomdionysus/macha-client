...
- Modern Web transformed playback uses hls.js/MSE with a bounded forward buffer; Pause stops managed HLS acquisition and Resume restarts it. One alternate transformed generation is prepared in the background and its manifest, initialization data and first segment are preflighted without attaching a second decoder. Fatal media recovery must demonstrate playback progress before another recovery is allowed, and fatal network restart is bounded before the preflighted alternate is promoted. Typed stream failures may trigger node failover; proven decoder or unsupported-source failures remain terminal without condemning a healthy endpoint. Samsung keeps its legacy native-HLS path.
- Direct Play read-ahead is optional. If its Service Worker is not already usable, playback takes the native URL immediately.
- Cluster Direct Play starts the primary source immediately and negotiates at most one byte-compatible standby in the background. The Service Worker retains one `video.src` and can continue an exact range at the next byte through that alternate after a retryable header or body failure; immutable media identity, size and MIME compatibility are required before registration. A failed speculative TCP/range read immediately tries and promotes a valid alternate, filling the cache before viewer demand reaches the failed node. Player status follows the worker's credential-free preferred stream origin.
