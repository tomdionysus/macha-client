...
function session(seekMs: number): PlaybackSession {
  return { sessionId: 's1', seekMs } as PlaybackSession;
}
...
describe('DirectSeekSessionSync', () => {
  it('coalesces rapid seeks and sends only the latest position', async () => {
    vi.useFakeTimers();
    const update = vi.fn(async (_sessionId: string, positionMs: number) => session(positionMs));
    const applied = vi.fn();
    const failed = vi.fn();
  ...
  it('serializes a newer seek behind an in-flight mutation and discards the stale response', async () => {
    vi.useFakeTimers();
    let resolveFirst: ((value: PlaybackSession) => void) | undefined;
    const update = vi.fn((_sessionId: string, positionMs: number) => {
      if (update.mock.calls.length === 1) {
        return new Promise<PlaybackSession>((resolve) => { resolveFirst = resolve; });
      }
