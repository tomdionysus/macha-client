export function needsEpisodeContextMigration(entry) {
    return entry.media?.kind === 'episode' && !entry.media.playbackContext;
}
export async function migrateEpisodeContext(api, entry) {
    const media = entry.media;
    if (!media || media.kind !== 'episode' || media.playbackContext)
        return entry;
    if (!media.parentId) {
