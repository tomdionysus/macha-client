...
const isSamsungBuild = mode === 'samsung';
const isAndroidBuild = mode === 'android';

/**
 * Traits knowable from the build target alone, before any `Platform`
 * instance exists — e.g. choosing a router at boot, which must happen
 * before `detectPlatform()` runs.
 */
export const buildPlatformTraits: Pick<PlatformTraits, 'usesDpadNavigation' | 'usesHashRouting' | 'receivesBackKeyEvents' | 'usesRemoteMediaControls'> = {
