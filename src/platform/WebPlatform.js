...
import { directPlayReadAheadMetrics, directPlayReadAheadUrl, releaseDirectPlayReadAhead, setDirectPlayReadAheadMode, } from '../playback/directPlayReadAhead';
function isLegacyWebVtt(url) {
    return /\.vtt(?:$|[?#])/i.test(url);
}
function validSubtitleManifest(value) {
...
class WebPlayer {
    ...
    async setSubtitle(subtitleUrl) {
        const video = this.video;
        if (!video)
            throw new Error('Player has no active media element');
    ...
    loadSubtitleTrack(video, url, generation) {
    ...
    async applySegmentedSubtitle(video, manifestUrl, manifest, generation) {
        const starts = [];
        let total = 0;
        ...
        const segmentCues = new Map();
        const loading = new Map();
        const wanted = new Set();
        const segmentUrl = (index) => new URL(`segment-${index}.vtt`, manifestUrl).toString();
        const unload = (index) => {
        ...
        const parseSegment = (index, text) => new Promise((resolve, reject) => {
        ...
        const load = (index) => {
            if (index < 0 || index >= starts.length || generation !== this.subtitleGeneration)
                return Promise.resolve();
            if (segmentCues.has(index))
                return Promise.resolve();
            ...
            const promise = (async () => {
                ...
                if (generation !== this.subtitleGeneration || !wanted.has(index))
                    return;
                let cues;
                try {
        ...
        const events = ['loadedmetadata', 'timeupdate', 'seeking', 'seeked'];
        for (const event of events)
            video.addEventListener(event, refresh);
        this.subtitleCleanup = () => {
            for (const event of events)
                video.removeEventListener(event, refresh);
            wanted.clear();
            for (const index of [...segmentCues.keys()])
                unload(index);
            loading.clear();
        ...
        for (let attempt = 0; attempt < 2; attempt += 1) {
            ...
            if (generation !== this.subtitleGeneration)
                return;
            if (segmentAt(video.currentTime * 1000) === current)
                break;
        }
        refresh();
    }
    async applySubtitle(video, subtitleUrl) {
    ...
    attachMediaDiagnostics(video) {
        ...
        for (const name of stateEvents) {
            video.addEventListener(name, () => {
                if (name === 'playing')
                    setDirectPlayReadAheadMode(this.directReadAheadSourceUrl, 'playing');
                else if (name === 'seeking')
                    setDirectPlayReadAheadMode(this.directReadAheadSourceUrl, 'seeking');
                else if (name === 'seeked')
                    setDirectPlayReadAheadMode(this.directReadAheadSourceUrl, video.paused ? 'paused' : 'playing');
