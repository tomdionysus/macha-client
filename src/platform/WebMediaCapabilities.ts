...
export function detectWebMediaCodecCapabilities(probe: MimeProbe): WebMediaCodecCapabilities {
  const videoCodecs: string[] = [];
  if (probe('video/mp4; codecs="avc1.42E01E"')) videoCodecs.push('h264');
  if (anySupported(probe, [
    'video/mp4; codecs="hev1.1.6.L93.B0"',
    'video/mp4; codecs="hvc1.1.6.L93.B0"',
  ])) videoCodecs.push('hevc');
  if (probe('video/webm; codecs="vp9"')) videoCodecs.push('vp9');
  if (probe('video/mp4; codecs="av01.0.05M.08"')) videoCodecs.push('av1');
  ...
  if (probe('audio/ogg; codecs="vorbis"')) audioCodecs.push('vorbis');
  ...
  if (probe('audio/mpeg')) audioCodecs.push('mp3');
  if (probe('audio/flac')) audioCodecs.push('flac');
