...
const api = {
    details: vi.fn(async (id) => {
        if (id === 'episode')
            return { id, kind: 'episode', title: 'E', mediaIds: [], parentId: 'season' };
        if (id === 'season')
            return { id, kind: 'season', title: 'S', mediaIds: [], parentId: 'series', showId: 'series', seasonNumber: 1, episodes: [] };
        throw new Error(id);
    }),
};
