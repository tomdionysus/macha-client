...
function catalogueItem(id, kind, partial = {}) {
    return {
        id,
        kind,
        title: id,
...
class FakeCatalogue {
    ...
    get(id) {
        if (id === 'show')
            return Promise.resolve(catalogueItem('show', 'show', { title: 'Show' }));
        if (id === 'season-1')
        ...
        if (id === 'artist-1')
            return Promise.resolve(catalogueItem('artist-1', 'artist', { title: 'Artist' }));
        if (id === 'album-1')
...
describe('MachaMediaApi', () => {
    it('loads only season summaries for a series detail page', async () => {
        ...
        if (details.kind !== 'show' || !('seasons' in details))
            throw new Error('expected show details');
    ...
    it('loads episode details only when the season page is opened', async () => {
        ...
        if (details.kind !== 'season' || !('episodes' in details))
            throw new Error('expected season details');
    ...
    it('maps artist, album and track hierarchy for the Music client', async () => {
        ...
        if (artist.kind !== 'artist' || !('albums' in artist))
            throw new Error('expected artist details');
        expect(artist.albums.map((item) => item.id)).toEqual(['album-1']);
        const album = await api.details('album-1');
        expect(album.kind).toBe('album');
        if (album.kind !== 'album' || !('tracks' in album))
            throw new Error('expected album details');
