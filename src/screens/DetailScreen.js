...
export function DetailScreen({ api, itemId, onBack, onPlay, onPlayFromStart, progress, onEdit }) {
    ...
    useEffect(() => {
        if (details.value && (details.value.kind === 'movie' || details.value.kind === 'episode' || details.value.kind === 'track'))
