...
- **Each node's cost of starting a stream is measured by this viewer.** From the moment core asks for a session to the first media fragment, per node, latest figure only and unknown after ten minutes. Measured live at 1.8 s on fi-1 and 20.3 s on gbni-1 for the same transcode. Not yet acted on: the move needs core to lead the requested position by it, which is waiting on core and on Tom.
