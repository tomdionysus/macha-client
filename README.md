...
Grid/rail artwork uses a monotonic viewport-demand policy. A single application-wide proximity registry, backed by the browser's own IntersectionObserver with a 1000 px preload margin, fires exactly once per card as soon as it comes near the viewport — including when a layout change elsewhere on the page reveals it, not only a scroll or resize. Once admitted, an artwork request is allowed to finish even if the card subsequently scrolls away or unmounts. Duplicate demand is coalesced by immutable artwork ID and completed Blobs remain cached for later mounts. There is no client-side concurrency limit on artwork transfers; the browser's own HTTP stack manages that. Detail/backdrop view consumers remain independently cancellable without discarding useful completed background work.
